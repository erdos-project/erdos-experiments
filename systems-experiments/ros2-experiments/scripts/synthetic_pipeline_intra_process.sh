...
for i in {1..1}
do
    filename="$TMP_DIR/synthetic-pipeline-$i-intra-process.csv"    
    tmp_files+=("$filename")
    ...
    args+=(
        ...
        -p "pipeline_${i}_segmentation:output_topic:=pipeline_${i}_lidar"
        ...
        # Tmp Planning
        -p "pipeline_${i}_tmp_planning:left_topic:=pipeline_${i}_fusion"
        -p "pipeline_${i}_tmp_planning:right_topic:=pipeline_${i}_localization"
        ...
        # Control
        -p "pipeline_${i}_planning:left_topic:=pipeline_${i}_planning"
        -p "pipeline_${i}_planning:right_topic:=pipeline_${i}_localization"
        -p "pipeline_${i}_planning:output_topic:=pipeline_${i}_control"
        -p "pipeline_${i}_planning:msg_size:=$CONTROL_MSG_SIZE"
        # Receiver
        -p "pipeline_${i}_receiver:recv_topic:=pipeline_${i}_control"
        -p "pipeline_${i}_receiver:is_inter_process:=false"
    )

    ros2 run latency synthetic_pipeline $i \
        --ros-args \
    ...
    python3 ../scripts/add_column_to_csv.py \    
            -i $filename -o $filename -c "pipeline_copies" -v $i
done
